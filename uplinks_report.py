#!/usr/bin/env python3
"""
Устройства по тегу border: Primary IPv4, колонка Netbox (interface + description),
колонка SSH (interface + description с устройства по SSH).
Подключение по имени хоста: device.name + SSH_HOST_SUFFIX (по умолчанию .3hc.io).
Тип устройства (Juniper/Arista) берётся из NetBox: device.platform.name (JunOS/Juniper → Juniper, Arista EOS → Arista).
Juniper: show interfaces descriptions | display json | no-more
Arista: show interfaces description | json или по каждому интерфейсу show int <name> description | json.
Переменные: NETBOX_URL, NETBOX_TOKEN, SSH_USERNAME, SSH_PASSWORD, SSH_HOST_SUFFIX, PARALLEL_DEVICES. DEBUG_SSH_JSON=1 — выводить в консоль JSON, полученный по SSH.
"""
...
# --- извлечение JSON из вывода с приглашением и мусором ---
...
def get_ssh_uplinks(
    ...
    Тип устройства берётся из platform_name (NetBox device.platform.name): JunOS/Juniper → Juniper, Arista EOS → Arista.
    Если platform_name не задан — по баннеру после входа (JUNOS в выводе).
    log(сообщение) — опциональный callback для вывода хода работы.
    debug_json — при True выводить в лог извлечённый JSON (или сырой вывод, если JSON не распарсился).
    """
    def _log(msg):
        if log:
            log(msg)
    ...
    def read_until_json_and_prompt(max_wait=120):
        """Читать вывод до появления приглашения и валидного JSON (по первой {)."""
        buf = []
        deadline = time.monotonic() + max_wait
    ...
    if not is_juniper and netbox_interface_names:
        # Arista: запрос по каждому интерфейсу из NetBox
        ...
        # Juniper или Arista без списка интерфейсов — одна команда на все
        if is_juniper:
...
def main():
    ...
    print("Загрузка списка устройств (tag=border)...", flush=True)
    devices = list(nb.dcim.devices.filter(tag="border"))
    if not devices:
        print("Устройств с тегом 'border' не найдено")
        return 0
    ...
    # выравнивание колонок (многострочные ячейки считаем по максимальной длине строки в ячейке)
    def cell_width(cell):
