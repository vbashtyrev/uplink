"""zabbix_sync_commit_rate main with --create-link-triggers."""
...
def test_main_create_link_triggers(monkeypatch, zabbix_env, netbox_env, tmp_path, capsys):
    ...
    mocker.on("item.get", lambda p: []).on("item.delete", lambda p: True)
    ...
    monkeypatch.setattr(mod, "ensure_simple_threshold_trigger", lambda *a, **k: (True, None))
    monkeypatch.setattr(mod, "ensure_simple_warn_trigger", lambda *a, **k: (True, None))
    monkeypatch.setattr(mod, "ensure_burst_sla_breach_trigger", lambda *a, **k: (True, None))
    ...
    assert "Done" in out or "OK:" in out
