...
Скрипты читают область мониторинга из NetBox и настраивают Zabbix.
Получение текущих показателей с устройств выполняется другим проектом.
...
| Скрипт | Описание |
|--------|----------|
| `run_uplinks_full.py` | **Один запуск** — чтение полной области из NetBox и настройка Zabbix (макросы, триггеры, агрегаты, карта, дашборды, сервисы); отчёт о работе и ошибках. Ключ **`--plan`** даёт предварительный отчёт без записи. |
| `netbox_uplinks_inventory.py` | Только чтение NetBox: обход Provider → Circuit → терминация стороны A → кабель → интерфейс на устройстве с тегом `border`. В мониторинг входят только Circuit типа `Uplink` со статусом `Active`. |
...
В рабочем режиме NetBox является единственным источником постоянных данных о
подключениях:
...
Скрипт не создаёт и не удаляет эти объекты. Он читает готовую цепочку и
настраивает мониторинг. Текущие показатели устройств получает другой
проект.
...
0. **Цепочки из NetBox**
   ...
   Этот inventory задаёт область мониторинга.

1. **Синхронизация с Zabbix**
   По полной области NetBox и Zabbix API выполняются макросы, триггеры,
   агрегаты, карта, дашборды и сервисы. Файл `dry-ssh.json` не нужен.

2. **Синхронизация макросов commit rate в Zabbix**
...
**Итого:** NetBox inventory → **`zabbix_sync_commit_rate.py`** →
**`zabbix_provider_aggregate.py`** → **`zabbix_map.py --update-map`** →
дашборды → **`zabbix_provider_services.py`**. Всё это делает
...
Описание интерфейса не определяет Provider и не включает канал в мониторинг.
Тег `automatization` не влияет на область мониторинга.

### Как добавить Provider

1. В NetBox откройте `Circuits → Providers` и создайте поставщика.
2. Заполните имя поставщика.
3. Заполните `aggregate_limit_gbps`, если для поставщика нужен общий лимит
   трафика. Поле `slo_percent` заполните только если целевой уровень
   доступности отличается от общего значения проекта.

### Как добавить Circuit

1. В NetBox откройте `Circuits → Circuits` и создайте канал у нужного Provider.
2. Укажите уникальный в рамках Provider идентификатор канала, тип `Uplink` и
   статус `Active`.
3. Заполните `billing_model`. Для обычного индивидуального лимита заполните
   `commit_rate` в Кбит/с. Для `FlatAggCap` индивидуальный `commit_rate` не
   задаётся: общий лимит хранится у Provider в `aggregate_limit_gbps`.
4. Создайте терминацию стороны A и соедините её кабелем с интерфейсом устройства
   с тегом `border`. Кабель может проходить через пару проходных портов.
5. Проверьте цепочку без изменений:

   ```bash
   python netbox_uplinks_inventory.py --json --dry-run
   ```

   Канал должен попасть в `complete`, а не в `incomplete`.

### Как отключить старый Circuit

1. В NetBox измените статус Circuit на `Offline` или `Decommissioned`.
   Сам объект не удаляйте.
2. Сначала выполните `netbox_uplinks_inventory.py --json --dry-run`, затем
   `run_uplinks_full.py --plan`.
3. Проверьте отчёт Zabbix и только после этого выполняйте рабочий запуск.
   Старые объекты NetBox автоматически не удаляются. `run_uplinks_full.py`
   пересобирает рабочие объекты Zabbix по оставшимся каналам. Скрипт
   `zabbix_uplinks_cleanup.py` очищает весь набор объектов uplinks, а не один
   Circuit, поэтому используйте его только для полной ручной очистки и сначала
   запускайте `zabbix_uplinks_cleanup.py --dry-run`. Если отключается последний
   полный Circuit, рабочий запуск остановится без записи; отдельную полную
   очистку в этом случае выполняйте только после проверки её отчёта.
...
цепочки из NetBox, читает текущие объекты Zabbix и пишет отчёт о предполагаемых изменениях в
...
- отчёт останавливается с ошибкой, если чтение NetBox прошло не полностью или
  ...
  цепочкам;
- карты, дашборды и сервисы сравниваются в практическом режиме: проверяются
  основные объекты, их идентификаторы и ожидаемые связи. Координаты карты и
...
`skipped` — работа не будет выполнена или потенциальное изменение подавлено
из-за неизвестного состояния, `not_evaluated` — сравнение не выполнялось.

Отдельно тот же отчёт можно получить скриптом **`zabbix_uplinks_plan.py`**:

| Ключ | Описание |
|------|----------|
| `--inventory-file` | Взять цепочки из готового файла inventory и не обращаться к NetBox повторно |
...
| Ключ | Описание |
|------|----------|
| `--plan` | Предварительный отчёт без записи в NetBox и Zabbix |
| `--netbox-checks` | Отдельно запустить сверку из внешнего файла; не входит в обычный путь |
| `--dry-ssh FILE` | Файл для отдельной сверки NetBox с данными устройств; обычный путь его не использует |
| `--no-netbox-apply` | Не запускать отдельную сверку NetBox; обычный путь её и так не запускает |
...
# Переменные окружения для рабочего запуска
...
- Переменные `SSH_*` нужны только отдельному ручному инструменту
  `uplinks_stats.py`, не рабочему запуску.
...
| `--inventory-file FILE` | Опрашивать только устройства из цепочек NetBox (файл шага 0). Без этого ключа опрашиваются все устройства с тегом из `NETBOX_TAG`. Обычный полный запуск этот ручной SSH-инструмент не вызывает. Список интерфейсов на устройстве по-прежнему отбирается по тексту `Uplink:` в описании порта |
...
Раскладка карты считает каждый хост и провайдера слотом размером `200×200`
пикселей. Между видимыми областями элементов и подписей оставляется зазор
`8` пикселей; шаг между колонками — `200` пикселей, шаг строк в плотной
раскладке — `100` пикселей. Если элементы не помещаются в строке, блок
переносится ниже, а размер карты увеличивается в заданных пределах.
Дополнительно проверяются видимые области подписей элементов и подписей линий
с трафиком. При пересечении обновление карты блокируется; провайдеры получают
подпись сверху, а устройства — снизу.
...
- В блоке: провайдер получает подпись сверху, хосты — снизу; компоненты
  размещаются с шагом 200 px по колонкам и 100 px по строкам.
...
Для схем, где у провайдера общий лимит по всем каналам, лимит хранится в
NetBox у Provider в поле **`aggregate_limit_gbps`** (Гбит/с). Скрипт берёт
провайдеров и каналы из полных цепочек NetBox и создаёт хосты
**«Uplinks {Provider}»** в группе **`UPLINKS_AGGREGATE_GROUP`** с calculated
items (сумма Bits in/out по каналам). Для провайдера с лимитом создаются
**три** триггера:
...
| `--dashboard-by-location NAME` | Имя дашборда по локациям (по умолчанию `Uplinks (by location)`); пустая строка — не удалять |
| `--dashboard-by-provider NAME` | Имя сводного дашборда по провайдерам (по умолчанию `Uplinks by providers`); пустая строка — не удалять |
