...
Раскладка карты считает каждый хост и провайдера прямоугольником размером
`200×200` пикселей и оставляет между ними зазор `40` пикселей. Если элементы
не помещаются в строке, блок переносится ниже. Дополнительно проверяются
видимые области подписей элементов и подписей линий с трафиком. При
пересечении обновление карты блокируется; провайдеры получают подпись сверху,
а устройства — снизу.
